...
- **Edit copy in place; do not `deleteObject` + recreate just to change text.** Recreating an element resets its transform, wiping any spacing a collaborator hand-tuned on a shared deck. Use `settext()`. Reserve delete-and-recreate for when you must MOVE or RESIZE a text box, because `updatePageElementTransform` (ABSOLUTE mode) on an existing TEXT_BOX silently resets its `size` to a default square even on a success response, so recreate it at the new coordinates rather than transforming it.
